	...
	async prompt(
		...
		return await super.prompt(messages, {
			onCompletion(completion) {
				...
				CourseService.create(completion, messages[messages.length - 1].content, model, userId);
			}
		});
	}
}
